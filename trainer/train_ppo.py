...
from constants import CHECKPOINT_DIR, OUT_DIR, REWARD_PATH, RLAIF_DATASET_PATH as DATA_PATH
...
def get_raw_model(model):
    ...
    兼容:
    - 普通模型
    - DistributedDataParallel 包装后的模型
    - torch.compile 包装后的模型
    """
...
def optimizer_step(actor_scheduler, critic_scheduler):
    """
    PPO 参数更新步骤
...
def sync_old_actor(old_actor_model):
    """
    将当前 actor 参数同步到 old_actor_model

    old_actor_model 用于 PPO 的重要性采样比例 ratio。
    """
    actor_state = get_raw_model(actor_model).state_dict()
    old_actor_model.load_state_dict(
        {k: v.detach().cpu() for k, v in actor_state.items()}
    )
...
def ppo_train_epoch(
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with torch.no_grad():
            model_for_gen = get_raw_model(actor_model)
        ...
        values = value_seq[torch.arange(len(last_indices), device=args.device), last_indices]
        ...
        # labels 的第 0 个位置对应原序列第 1 个 token, 因此和 prompt_lengths 对齐时使用 >= prompt_lengths
        resp_mask = (
        ...
        # 梯度累计: PPO 也需要按 accumulation_steps 平均 loss
        loss = loss / args.accumulation_steps
        loss.backward()

        # 更新参数
        if step % args.accumulation_steps == 0:
            optimizer_step(actor_scheduler, critic_scheduler)
        ...
        if step % args.update_old_actor_freq == 0:
            sync_old_actor(old_actor_model)
...
if __name__ == "__main__":
    ...
    args.reward_model_path = os.path.abspath(os.path.expanduser(args.reward_model_path))
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = (
            f"ZzMind-PPO-Epoch-{args.epochs}-"
    ...
    if not os.path.exists(ckp):
        raise FileNotFoundError(
            f"找不到用于初始化 critic 的权重文件: {ckp}\n"
            f"请确认已经存在 {base_weight}_{lm_config.hidden_size}{moe_suffix}.pth, "
            f"或者检查 --save_dir / --hidden_size / --use_moe / --reasoning 参数。"
        )
    ...
    total_optimizer_steps = max(
        1,
        ((iters + args.accumulation_steps - 1) // args.accumulation_steps)
        * args.epochs,
    )
    ...
    # old_actor_model 和 ref_model 不参与训练, 但需要在当前 device 上推理
    old_actor_model.to(args.device)
