...
from model.ZzModel import ZzMindConfig, ZzMindForCausalLM
from model.model_lora import apply_lora, load_lora
from trainer.trainer_utils import setup_seed

import os
from constants import OUT_DIR, MODEL_DIR
...
def init_model(args):
    ...
    if "model" in args.load_from:
        ...
        ckp = os.path.join(
            args.save_dir,
            f"{args.weight}_{args.hidden_size}{moe_suffix}.pth"
        )
    ...
    print(
        f"ZzMind 模型参数量: {sum(p.numel() for p in model.parameters()) / 1E6:.2E} M(illion)"
    )
    return model.eval().to(args.device), tokenizer

def main():
    parser = argparse.ArgumentParser(
        description="ZzMind 模型推理与对话"
    )
    parser.add_argument(
        "--load_from",
        default=MODEL_DIR,
        type=str,
        help="模型加载路径(model 为原生 torch 权重, 其他路径为 transformers 格式)"
    ) 
    parser.add_argument(
        "--save_dir", 
        default=OUT_DIR, 
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="启用 RoPE 位置编码外推(4倍, 仅解决位置编码问题)",
    )
    parser.add_argument(
        "--max_new_tokens",
        default=8192,
