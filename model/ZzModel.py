...
def precompute_freqs(
    ...
    # 计算出的 freqs 就是标准的 1 / (base ** (2i / dim))

    freqs, attn_factor = (
        1 / (rope_base ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim)),
        1.0,
    )
    ...
    if rope_scaling is not None:
        ...
        # beta_fast (对应论文中的 alpha): 高频边界, 波长比例大于此值的维度不缩放
        # beta_slow (对应论文中的 beta): 低频边界，波长比例小于此值的维度全量缩放
        ...
        orig_max, factor, beta_fast, beta_slow, attn_factor = (
            ...
            rope_scaling.get("attention_factor", rope_scaling.get("attn_factor", 1.0)),
        )

        # 只有当要推断的长度大于原始训练长度时才应用缩放
        if end > orig_max:
            ...
            # 6. 频率融合公式：f'(i) = f(i) * ((1 - gamma) + gamma / s)
...
def apply_rotary_pos_emb(q, k, cos, sin, position_idx=None, unsqueeze_dim=1):
    def rotate_half(x):
        return torch.cat(
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        xk = xk.view(bsz, sql, self.n_local_kv_heads, self.head_dim)
        xv = xv.view(bsz, sql, self.n_local_kv_heads, self.head_dim)
...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training and self.alpha > 0.0:
            ...
            if self.seq_aux:
                scores_for_seq_aux = scores_for_aux.view(bsz, sql, -1)
                ce = torch.zeros(
...
class MoEFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            # 按照定义的 num_experts_per_tok 重复输入 token
            # 每个 token 安排 num_experts_per_tok 个专家处理
            ...
            # 加权求和
            # 最后的 y 的意义是每个 token 经过专家处理后的加权结果
...
class ZzMindModel(nn.Module):
    def __init__(self, config: ZzMindConfig):
        ...
        self.layers = nn.ModuleList(
            [ZzMindBlock(l, config) for l in range(self.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 计算 start_pos: 如果存在 past, 则 start_pos 为已有 past 序列长度
        start_pos = (
